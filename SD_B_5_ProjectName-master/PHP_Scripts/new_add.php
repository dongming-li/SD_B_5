...
if(!empty($upc)){
	...
	$sql = 'Select * FROM product'.$company_id.'__'.$location_id.' WHERE UPC='.$upc;
...
if($exists == 0) {
	...
	if(!empty($upc) || !empty($name) || !empty($quantity) || !empty($bin_id) || !empty($location_id)) {			
		$sql = 'INSERT INTO product'.$company_id.'_'.$location_id.' (UPC,PRODUCT_NAME,QUANTITY,DESCRIPTION,BIN_ID,MANUFACTURER_PRICE,MSRP,LOCATION_ID, ARCHIVE) VALUES ('.$upc.',"'.$name.'","'.$current_quantity.'","'.$description.'","'.$bin_id.'","'.$manufacturer_price.'","'.$msrp.'","'.$location_id.'",0)';
	 ...
	 $sql = 'UPDATE product'.$company_id.'_'.$location_id.' SET QUANTITY="'.$current_quantity.'" WHERE UPC='.$upc;
...
if($conn->query($sql))
{
	$sql = 'SELECT * FROM product'.$company_id.'_'.$location_id.' WHERE UPC='.$upc;
	if($result = $conn->query($sql))
	{	
		if($result->num_rows > 0) 
		{
